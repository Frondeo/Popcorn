...
//------------------------------------------------------------------------------------------------------------
ABall::ABall()
	: Ball_State(EBS_Normal), Ball_Pen(0), Ball_Brush(0), Ball_X_Pos(0.0), Ball_Y_Pos(181.0), Ball_Speed(3.0), Ball_Direction(M_PI - M_PI_4), Ball_Rect{}, Prev_Ball_Rect{}
{
}
//------------------------------------------------------------------------------------------------------------
...
void ABall::Move(ALevel *level, int platform_x_pos, int platform_width)
{
	...
	if (next_y_pos > max_y_pos)
	{
		...
		{
			if (next_y_pos > (double)max_y_pos + (double)AsConfig::Ball_Size * 2.0) //Проверка шарика
			Ball_State = EBS_Lost;
		}
	}
